...
public class UserService {
    ...
    public LoginVO login(LoginFrom from) {
        ...
        // 4. 校验账号状态
        if (user.getStatus() == 0){
            throw new ServiceException("您的账号被冻结 请联系管理员");
        }
        // 5. 生成JWT令牌
        String token = jwtUtil.generateToken(user.getId().toString(), user.getUsername());
    ...
    public LoginVO refreshToken(String authHeader) {
        // 1. 从 header 里抠出 token
        if (authHeader == null || !authHeader.startsWith("Bearer ")){
            throw new ServiceException("token格式错误");
        }
        String token = authHeader.substring(7);

        // 2. 解析 token（必须当前有效，过期的不给续）
        Long userId;
        String username;
        try {
            Claims claims = jwtUtil.parseToken(token);
            ...
            throw new ServiceException("token 无效或者已经过期 请重新登录");
        }
        // 3. 检查黑名单（登出过的 token 不能续）
        ...
        // 4. 生成新 token
        String newToken = jwtUtil.generateToken(userId.toString(), username);

        // 5. 刷新 Redis 缓存（延长过期时间）
        String userJson = stringRedisTemplate.opsForValue().get("user:info" + userId);
        if (userJson != null) {
            stringRedisTemplate.expire("user:info" + userId, 86400, TimeUnit.SECONDS);
        }
         ...
         *   │
         *   ▼
         * 前端 POST /api/users/refresh  Header: Bearer eyJhbG...
         *   │
         *   ▼
         * UserService.refreshToken()
         *   ├─ 解析 token → userId=5, username="lx"
         *   ├─ 查 Redis 黑名单 → 没被登出
         *   ├─ 生成新 token（重新计时 24h）
         *   ├─ 刷新 Redis 缓存 TTL
         *   └─ 返回新 token
    ...
    public void deleteAccount(String authHeader) {
        ...
        if (authHeader != null && authHeader.startsWith("Bearer ")) {
            String token = authHeader.substring(7);
            stringRedisTemplate.opsForSet().add("token:blacklist", token);
            stringRedisTemplate.expire("token:blacklist", 86400, TimeUnit.SECONDS);
        }
    }

    public List<FollowingVO> batchGetUsers(String idsStr) {
        // 1. 解析逗号分隔的 ID 列表
        ...
        // 2. 批量查用户
        Map<Long, User> userMap = userDao.selectBatchIds(ids)
                .stream().collect(Collectors.toMap(User::getId, u -> u));
    ...
    public void bindPhone(BindPhoneFrom form) {
        ...
        // 1. 校验验证码
        smsService.verifyCode(form.getPhone(), form.getCode());
