...
import cn.lx.worldcoffee.module.user.domain.form.*;
import cn.lx.worldcoffee.module.user.domain.vo.*;
import cn.lx.worldcoffee.module.user.service.SmsService;
...
public class UserController {
    ...
    @Operation(summary = "刷新 Token", description = "用当前有效的 JWT 换取新 Token，延长有效期")
    @PostMapping("/refresh")
    ...
    @Operation(summary = "注销账号", description = "软删除当前用户账号，status 置为 0，JWT 失效")
    @DeleteMapping("/me")
    public Result<Void> deleteMe(@RequestHeader("Authorization") String authHeader) {
        userService.deleteAccount(authHeader);
        return Result.success(null);
    }

    @Operation(summary = "批量查询用户", description = "根据用户ID列表批量查询用户基本信息，用于 @ 提及等场景")
    @GetMapping("/batch")
    public Result<List<FollowingVO>> batchUsers(
            @Parameter(description = "用户ID列表，逗号分隔") @RequestParam("ids") String idsStr) {
        return Result.success(userService.batchGetUsers(idsStr));
    }

    //方案 B（开发调试用） — 把 code 返回给前端，方便你测试时不用等短信：
    @Operation(summary = "发送验证码", description = "向指定手机号发送 6 位验证码")
    @PostMapping("/sms/code")
    public Result<String> sendSmsCode(@RequestParam String phone) {
        String s = smsService.sendCode(phone);
        return Result.success(s);
    }

    @Operation(summary = "绑定手机号", description = "验证码校验通过后修改绑定手机号")
    @PutMapping("/me/phone")
